...
| `host_api_url` | `http://192.168.0.109:6109` | the direct origin the server itself uses in `call_host()` |

Both scripts took `host_url` and prefixed the backend server's origin:

```python
host_url = server_url.rstrip("/") + host_url   # http://192.168.0.103:5109/host/host-clone-1
```
...
```
http://192.168.0.103:5109/host/host-clone-1/health  -> 404
http://192.168.0.109:6109/health                    -> 200
```
...
Add a CI job that runs the `vpt/smoke_*` scripts against the deployed environment.
They exist precisely to catch this class of breakage and currently catch nothing —
two of five were broken for an unknown length of time with no signal.
