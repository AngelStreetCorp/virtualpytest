...
| VNC on every host | **generated per install** since 2026-09-15 (`HOST_VNC_PASSWORD`) — older installs carry the old shared default | `HOST_VNC_PASSWORD` in `backend_host/src/.env`, then rewrite the password file: `echo "$HOST_VNC_PASSWORD" \| vncpasswd -f > ~vpt_user/.vnc/passwd` (what `install_host.sh` does — on Debian/Raspberry Pi OS the binary is `tigervncpasswd`, and the file must end up non-empty or VNC rejects every connection) and restart the VNC unit | the `vnc_lite_*` pages read it from the host `.env`, nothing else to touch |
