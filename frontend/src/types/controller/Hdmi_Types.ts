...
/** A single captured frame for the video-capture playback strip. */
export interface VideoFrame {
  url: string;
  timestamp: number; // ms since epoch (file mtime)
}
...
export interface HdmiStreamActions {
  ...
  // Fetch the captured frames recorded between startMs and endMs (inclusive),
  // populate videoFrames + totalFrames, and reset to the first frame.
  fetchCapturedFrames: (startMs: number, endMs: number) => Promise<void>;
