...
export function useRemoteConnection(remoteType: RemoteType) {
  ...
  const { selectedHost } = useHostControl();
  ...
  const { postToHost, executeRemoteCommand } = useControllerApi(selectedHost);
  ...
  const handleScreenshot = useCallback(async () => {
    ...
    try {
      ...
      const result = await postToHost('/server/remote/takeScreenshot');
  ...
  const handleScreenshotAndDumpUI = useCallback(async () => {
    ...
    try {
      ...
      // Use direct server route call instead of proxy
      const result = await postToHost('/server/remote/screenshotAndDump');
  ...
  const handleGetApps = useCallback(async () => {
    ...
    try {
      ...
      // Use direct server route call instead of proxy
      const result = await postToHost('/server/remote/getApps');
  ...
  const handleClickElement = useCallback(
    async (element: AndroidElement) => {
      ...
      try {
        ...
        const result = await executeRemoteCommand('click_element', {
  ...
  const handleRemoteCommand = useCallback(
    async (command: string, params: any = {}) => {
      ...
      try {
        ...
        if (remoteType === 'android-mobile' && command === 'LAUNCH_APP') {
          const result = await executeRemoteCommand('launch_app', { package: params.package });
        ...
        const result = await executeRemoteCommand('press_key', { key: command });
    ...
    [deviceConfig, remoteType, executeRemoteCommand],
  );
