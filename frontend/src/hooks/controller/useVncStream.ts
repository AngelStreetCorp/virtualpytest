...
import { DragArea, VncStreamState, VncStreamActions, VideoFrame } from '../../types/controller/Vnc_Types';
...
export function useVncStream({
  ...
  const { postToHost } = useControllerApi(host, 'host');
  ...
  const handleTakeScreenshot = useCallback(async () => {
    ...
    try {
      ...
      const result = await postToHost(
        ...
        {},
        { includeDeviceId: true, deviceIdOverride: 'host' },
      );
  ...
  // Persist + load captured frames for playback: promoteCapturedFrames copies the
  // recording window from HOT (RAM, rotating) to COLD (persistent) storage — the
  // same hot->cold promotion takeScreenshot does — so frames don't vanish when the
  // RAM dir rotates. Host selects by duration from its newest frame, oldest-first.
  const fetchCapturedFrames = useCallback(
    async (startMs: number, endMs: number) => {
      try {
        ...
        setCurrentFrame(0);
        console.log(`[@hook:useVncStream] Loaded ${captures.length} captured frames for playback`);
      } catch (error) {
        console.error('[@hook:useVncStream] Failed to fetch captured frames:', error);
        setVideoFrames([]);
        setTotalFrames(0);
      }
    },
