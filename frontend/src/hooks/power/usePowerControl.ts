...
export const usePowerControl = ({
  ...
  const getPowerStatus = useCallback(async () => {
    ...
    try {
      const data: PowerStatusResponse = await api.post(buildServerUrl('/server/power/getStatus'), {
  ...
  const executePowerCommand = useCallback(
    async (command: PowerCommand): Promise<boolean> => {
      ...
      try {
        const data: PowerControlResponse = await api.post(buildServerUrl('/server/power/executeCommand'), {
          host_name: host.host_name,
          device_id,
          command,
          params: {},
