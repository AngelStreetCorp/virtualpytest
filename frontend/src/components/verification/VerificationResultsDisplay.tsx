        ...
        <Box sx={{ mb: 2 }}>
          {testResults.map((result, index) => (
            ...
            >
              ...
              {/* Evidence report link: success report for PASS (false-positive review),
                  failure report for FAIL. Both are full HTML+PNG evidence packs. */}
              {(result.success ? result.success_report_url : result.debug_report_url) && (
                <Typography
                  variant="caption"
                  sx={{ fontSize: '0.7rem', display: 'block', mt: 0.5 }}
                >
                  <a
                    href={result.success ? result.success_report_url : result.debug_report_url}
                    target="_blank"
                    rel="noreferrer"
                    style={{ color: result.success ? '#4caf50' : '#f44336' }}
                  >
                    {result.success ? 'View success report' : 'View failure report'}
                  </a>
                </Typography>
              )}
            </Box>
