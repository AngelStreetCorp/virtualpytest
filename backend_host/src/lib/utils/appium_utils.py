...
# Defers all annotations to strings (PEP 563) so they're never evaluated at
# runtime. Needed because method signatures below reference `webdriver.Remote`
# unconditionally, but `webdriver` only exists when the `appium` package is
# installed (see the guarded import below) — without this, importing this
# module on a host without Appium installed raises NameError at class
# definition time (discovered 2026-09-07 backfilling non-regression tests).
