...
Reads YAML files from docs/api/specs/ and generates HTML documentation
"""
...
# Configuration
YAML_DIR = Path(__file__).parent.parent / "docs" / "api" / "specs"
DOCS_DIR = Path(__file__).parent.parent / "docs" / "api" / "docs"
# redoc-cli was removed from frontend/package.json in b37bd2f85 (2026-07-15):
# it pulled in 31 npm vulnerabilities (8 critical, 10 high) via transitive deps
# and wasn't referenced anywhere except this script. It is intentionally NOT a
# devDependency anymore — don't `npm install` it back into frontend/.
# If a local copy happens to be present (e.g. installed manually), prefer it —
# no registry fetch needed. Otherwise fall back to a one-off `npx`, which
# fetches redoc-cli into npm's cache for this invocation only and never
# touches frontend/package.json or its lockfile.
REDOC_BIN = Path(__file__).parent.parent / "frontend" / "node_modules" / ".bin" / "redoc-cli"
REDOC_NPX_PACKAGE = "redoc-cli@0.13.21"
...
def get_redoc_command():
    """Return the argv prefix to invoke redoc-cli's `bundle` command, or None.

    Prefers a locally-installed binary (no registry fetch); falls back to a
    one-off `npx` invocation (requires network access, doesn't persist
    anything to frontend/package.json).
    """
    if REDOC_BIN.exists():
        return [str(REDOC_BIN)]
    if shutil.which("npx"):
        return ["npx", "--yes", REDOC_NPX_PACKAGE]
    return None


def check_node_tools():
    """Verify redoc-cli can be invoked, either locally or via npx."""
    if REDOC_BIN.exists():
        return True
    if shutil.which("npx"):
        print(f"{Colors.YELLOW}⚠{Colors.NC} redoc-cli not found at {REDOC_BIN}")
        print(f"   Falling back to `npx --yes {REDOC_NPX_PACKAGE}` (one-off, requires network).")
        return True
    print(f"{Colors.RED}✗{Colors.NC} Neither a local redoc-cli nor npx is available.")
    print(f"   redoc-cli is intentionally NOT installed as a devDependency (removed in")
    print(f"   b37bd2f85 for 31 npm vulnerabilities). Install Node.js/npm to get npx,")
    print(f"   which fetches it one-off without adding it back to frontend/package.json.")
    return False
...
def generate_html_docs():
    ...
    for index, yaml_file in enumerate(yaml_files, 1):
        ...
        spec_title = get_spec_title(yaml_file)
        
        print(f"{Colors.BLUE}[{index}/{total}]{Colors.NC} {spec_name}.yaml → {spec_name}.html")
        
        try:
            # Call the local redoc-cli binary if present (no registry fetch, can't
            # hang on a cold cache); otherwise npx fetches it one-off. timeout is
            # a longer backstop in the npx case since that path does hit the network.
            timeout = 120 if redoc_cmd[0] != "npx" else 180
            subprocess.run([
                *redoc_cmd, 'bundle',
                str(yaml_file),
                '-o', str(html_file),
                '--title', f'VirtualPyTest API - {spec_title}'
            ...
            print(f"  {Colors.RED}✗{Colors.NC} Timed out after 120s rendering {spec_name}. Skipping.\n")
...
<body>
    <div class="container">
        <div class="section">
            <h2>📡 Server APIs ({len(server_specs)})</h2>
            <div class="api-grid">
"""
    
    for spec_file in sorted(server_specs):
        name = get_spec_title(spec_file).replace('SERVER - ', '', 1).strip()
...
def get_spec_title(yaml_file: Path) -> str:
    """Read OpenAPI info.title from the spec, with a filename fallback."""
    try:
        ...
        pass
    return spec_name_to_title(yaml_file.stem)

def spec_name_to_title(spec_name: str) -> str:
    """Fallback display title derived from the filename."""
    label = spec_name.replace('server-', '').replace('-', ' ').title()
