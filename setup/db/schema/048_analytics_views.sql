-- 048_analytics_views.sql
-- Aggregates behind Monitoring > Analytics (/monitoring/analytics). One object per
-- tab section. Canonical definition; the dated copy in setup/db/migrations/ applies
-- the same thing to a database that already exists. Idempotent: safe to re-run.
--
-- WHY THESE ARE VIEWS AND NOT QUERIES IN THE APP
-- The Supabase client has no GROUP BY, so every aggregate here would otherwise be
-- done by pulling rows into Python. system_device_metrics is ~926k rows and
-- system_metrics ~796k, and BUG-0074 is what happens when dashboard aggregates scan
-- them unbounded (Postgres shared memory exhausted). Every object below is
-- time-bounded and rides an existing index on `timestamp`.
--
-- WHY TWO OF THEM ARE MATERIALIZED (measured on the live DB, 2026-09-17)
--   analytics_device_status              81 ms   -> plain view (was 252 ms before §0)
--   analytics_host_status                48 ms   -> plain view (was 140 ms)
--   analytics_alerts_daily               19 ms   -> plain view
--   analytics_incidents_daily            25 ms   -> plain view
--   analytics_script_runs_daily         369 ms   -> MATERIALIZED, now 5.6 ms
--   analytics_device_availability_daily 1382 ms  -> MATERIALIZED, now 1.4 ms
-- The two live-status views stay plain on purpose: they answer "is this device up
-- right now", and a 15-minute-stale answer to that is worse than an 81 ms fresh one.
-- They sit behind the server's 30 s pre-warmed cache, so no user ever waits on them.
--
-- SECURITY. Plain views use security_invoker so RLS on the base tables still applies
-- (same convention as device_availability_summary in 007_system_monitoring_tables.sql).
-- Materialized views cannot do that and do not carry RLS, so they are granted to
-- service_role only and explicitly revoked from anon/public — see TASK-10, the
-- service_role lockdown. The server reads with service_role.
