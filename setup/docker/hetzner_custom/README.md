...
- Setup automatically copies `config.env.example` → `config.env` on first run
...
- `api.virtualpytest.com` → Your Hetzner server IP (e.g., `203.0.113.10`)
...
**What setup.sh does:**
- ✅ Copies `config.env.example` → `config.env` (if not exists)
- ✅ Generates nginx config with N hosts (based on `HOST_MAX`)
- ✅ Generates `docker-compose.yml` with optional Grafana
- ✅ Creates `backend_host_*/.env` files for each host
- ✅ Configures VPN if enabled
- ✅ Sets up swap memory
...
│  Port: 6109    │            │  Port: 6110    │ ... │ (+ more hosts)
...
│  - Enable via ENABLE_GRAFANA=true               │
└─────────────────────────────────────────────────┘
```

**Key Features:**
- 🔢 **Dynamic Hosts**: Deploy 1-10 hosts (set via `HOST_MAX` in config.env)
- 📊 **Optional Grafana**: Enable/disable per server (set via `ENABLE_GRAFANA`)
- 🌐 **Optional VPN**: WireGuard support (set via `USE_VPN`)
- 🔧 **Auto-generated configs**: Nginx, docker-compose, and host .env files
